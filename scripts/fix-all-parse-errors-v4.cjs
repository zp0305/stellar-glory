...
function applyGlobalFixes(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Pattern 10: "answer": ", "explanation": """ → "answer": "", "explanation": ""
    line = line.replace(/"answer":\s*",\s*"explanation":\s*"""/g, '"answer": "", "explanation": ""');
    line = line.replace(/'answer':\s*',\s*'explanation':\s*'''/g, "'answer': '', 'explanation': ''");

    // Pattern 11: 行中有 """ 三个引号 → 修复为 "" （空字符串）
  ...
  while (iteration < maxIterations) {
    ...
    for (const fix of uniqueFixes) {
      ...
      if (fix.code === 1005) {
        ...
        // 检查是否是 "answer": ", "explanation": 模式
        if (line.includes('"answer":') && line.includes('"explanation":')) {
          const newLine = line.replace(/"answer":\s*",\s*"explanation":\s*"/, '"answer": "", "explanation": "');
          if (newLine !== line) {
