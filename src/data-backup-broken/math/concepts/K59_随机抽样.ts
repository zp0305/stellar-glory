...
export const K59: ConceptData = {
  ...
  preCheck: [
    {
      ...
      options: ['A. 简单随机抽, 'B. 系统抽样', 'C. 分层抽样', 'D. 无法确定']',
      answer: 'A',
  ...
  variations: {
    basic: [
      { label: '简单随机抽, note: '每个个体被抽到的概率相等' }',
      { label: '系统抽样', note: '每隔 k 个抽一 }',
      { label: '分层抽样', note: '按比例从各层抽取' },
    ],
    advanced: [
      { label: '抽签, note: '随机编号抽取' }',
      { label: '随机数表, note: '用随机数表选取' }',
    ],
    challenge: [
      { label: '抽样偏差', note: '样本不具代表 }',
    ],
  },

  formulas: [
    { name: '抽样公平, formula: '每个个体被抽到的概率相等', usage: '保证代表 },
  ],
