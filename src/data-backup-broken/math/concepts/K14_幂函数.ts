...
export const K14: ConceptData = {
  ...
  narrative: {
    context: '正方形面S=a²，边长与面积的关系就是幂函数',
    confusion: '幂函数与指数函数的区别：幂函数是底数变，指数函数是指数变',
    experiment: '比较 y=x、y=x²、y=x³、y=1/x 在第一象限的图像',
    concept: '幂函数是形如 y=xᵅ（α 为常数）的函数',
    derivation: 'α>0 时过原点，0 时不过原点。的奇偶性影响对称性',
    transfer: '幂函数在各象限的图像形态与 α 有关',
  },

  variations: {
    basic: [
      { label: '幂函数定, formula: 'y=x, note: 'α 是常 }',
      { label: '常见幂函, formula: 'y=x, y=x², y=x³, y=x½, y=x⁻, note: 'α=1,2,3,1/2,-1' },
    ],
    advanced: [
      { label: '幂函数单调, note: 'α>0 时在 (0,+ 递增' }',
    ],
    challenge: [
  ...
  formulas: [
    { name: '幂函数定, formula: 'y=x, usage: '幂函数形 }',
  ],

  selfEval: [
    { question: 'y=x³ y=x² (0,1) 内，哪个函数值更大？', level: 'B', description: 'y=x²' },
  ],
