...
export const K22: ConceptData = {
  ...
  preCheck: [
    {
      ...
      options: ['A. 偶函, 'B. 导函数有两个零点', 'C. 极值点不在区间中点', 'D. 函数图像关于 y 轴对],
      answer: 'C',
  ...
  narrative: {
    ...
    concept: '极值点偏移是指极值点 x₀ 不在 xx的中点位置',
  ...
  variations: {
    basic: [
      ...
      { label: '对称化构, note: '构造辅助函 },
    ],
    advanced: [
      { label: '换元, note: 't=x-x₀' }',
    ],
    challenge: [
      { label: '双变量处, note: '极值点偏移的典型应 },
    ],
  },

  formulas: [
    { name: '偏移判断', formula: 'f(x=f(x x₁≠x, usage: '判断偏移' }',
  ],

  selfEval: [
    { question: '极值点偏移问题主要用什么方法解决？', level: 'B', description: '对称化构造或换元 }',
  ],
