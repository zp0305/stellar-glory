...
export const sampleKnowledgePoints: KnowledgePoint[] = [
  {
    ...
    knowledgeNetwork: {
      ...
      edges: [
        { source: 'K001-1', target: 'K001-2', label: '差' },
        { source: 'K001-2', target: 'K001-3', label: '对比' },
      ],
    },
    methodology: {
  ...
  },
  {
    ...
    principle: {
      ...
      derivation: '速度是矢量，有大小和方向。\n平均速度为 \\bar{v} = \\frac{\\Delta x}{\\Delta t}$\n\n瞬时速度为 \\Delta t \\to 0$ 时的平均速度',
  ...
  },
  {
    ...
    methodology: {
      ...
      tips: ['加速度正为加速，负为减速（取决于正方向）', '加速度描述速度变化的快慢，不是变化的多少'],
...
export const sampleStrategies: Strategy[] = [
  {
    ...
    techniques: [
      'v-t 图像中，图像在 t 轴上方为正方向运动',
      'v-t 图像中，图像在 t 轴下方为反方向运动',
      '两图像交点为速度相等，常考追及问题',
    ],
    commonMistakes: [
  ...
  },
  {
    ...
    steps: [
      ...
      { order: 3, title: '速度相等', description: '令 $v_1 = v_2$，求 $t$ 和临界位置' },
      { order: 4, title: '比较位移', description: '比较相对位移与初始间距' },
      { order: 5, title: '判断结果', description: '$t$ 时若追者位移大于初始间距，则能追上' },
    ],
    techniques: [
      '速度大者追速度小者：一定能追上，追及时速度相等时刻距离最大',
...
const CAT_COLOR: Record<string, string> = {
  ...
  '机械振动': 'bg-amber-100 text-amber-700',
