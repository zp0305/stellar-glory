import { registerSubject, SubjectDataRegistry } from '@/data/registry';
import { allConcepts, conceptDataMap, conceptList, getConceptById } from './concepts';
import { allModels, modelDataMap, ALL_MODEL_IDS, biologyModels, getModelById } from './models';
import { formulas, formulaDataMap, getFormulaById } from './formulas';
import { questions, questionDataMap, getQuestionById, getQuestionsByModelId } from './questions';
import { allStrategies, strategyDataMap, getStrategyById } from './strategies';

const biologyRegistry: SubjectDataRegistry = {
  subjectId: 'biology',
  allConcepts,
  conceptDataMap,
  conceptList,
  getConceptById,
  allModels,
  modelDataMap,
  ALL_MODEL_IDS,
  modelChapters: biologyModels,
  getModelById,
  formulas,
  formulaDataMap,
  getFormulaById,
  questions,
  questionDataMap,
  getQuestionById,
  getQuestionsByModelId,
  strategies: allStrategies,
  strategyDataMap,
  getStrategyById,
};

registerSubject('biology', biologyRegistry);